...
bool WTagged_Randomize(float mass, float pT, float dR_lepton)
{
    if ((pT > 200) && (r0->Rndm() < 0.1)) return true;
    else return false;
}
...
bool Selector_highdMSignal()
{
    ...
    if ((myEventPointer->mStop == -1)
     ...
     && (myEventPointer->mStop <= 750)
     && (myEventPointer->mNeutralino >= 0)
     && (myEventPointer->mNeutralino <= 50))
      return true;
    else
      return false;
...
bool Selector_AtLeastOneHighPtAK8()
{
    for (int i = 0 ; i < 10 ; i++)
        if (myEventPointer->recoW_pT[i] > 200) return true;

    return false;
}

bool Selector_3jetsPlusHighPtAK8()        { if (Selector_AtLeastOneHighPtAK8())    { return Selector_3jets(); } else return false; }
bool Selector_3jetsExactlyPlusHighPtAK8() { if (Selector_AtLeastOneHighPtAK8())    { return Selector_3jetsExactly(); } else return false; }
bool Selector_4jetsPlusHighPtAK8()        { if (Selector_AtLeastOneHighPtAK8())    { return Selector_4jets(); } else return false; }
bool Selector_4jetsPlusVetoWtag()         { if (!Selector_AtLeastOneWTag())        { return Selector_4jets(); } else return false; }
bool Selector_4jetsPlusVetoHighPtAK8()    { if (!Selector_AtLeastOneHighPtAK8())   { return Selector_4jets(); } else return false; }
...
int main (int argc, char *argv[])
{
  ...
  // ##########################
     ...
     mySonic.AddRegion("presel",         "Preselection",            &Selector_presel);     mySonic.ScheduleVariablesForRegion("presel","all");
     mySonic.AddRegion("baseline",       "Baseline",                &Selector_baseline);   mySonic.ScheduleVariablesForRegion("baseline","all");
 
     // Reference
     mySonic.AddRegion("lowdM_MET>200",  "low DeltaM | MET > 200",  &Selector_lowdM_200);  mySonic.ScheduleVariablesForRegion("lowdM_MET>200", "all");
     mySonic.AddRegion("highdM_MET>200", "high DeltaM | MET > 200", &Selector_highdM_200); mySonic.ScheduleVariablesForRegion("highdM_MET>200", "all");

     mySonic.AddRegion("highdMSignal",      "highdMSignal",         &Selector_highdMSignal);      mySonic.ScheduleVariablesForRegion("highdMSignal","all");
     
     // >= 3 jets
     mySonic.AddRegion(">=3jets",           "",&Selector_3jets);                      mySonic.ScheduleVariablesForRegion(">=3jets","all");
     mySonic.AddRegion(">=3jets+Wtag",      "",&Selector_3jetsPlusWtag);              mySonic.ScheduleVariablesForRegion(">=3jets+Wtag","all");
     mySonic.AddRegion(">=3jets+highPtAK8", "",&Selector_3jetsPlusHighPtAK8);         mySonic.ScheduleVariablesForRegion(">=3jets+highPtAK8","all");
     
     // = 3 jets
     mySonic.AddRegion("=3jets",            "",&Selector_3jetsExactly);               mySonic.ScheduleVariablesForRegion("=3jets","all");
     mySonic.AddRegion("=3jets+Wtag",       "",&Selector_3jetsExactlyPlusWtag);       mySonic.ScheduleVariablesForRegion("=3jets+Wtag","all");
     mySonic.AddRegion("=3jets+highPtAK8",  "",&Selector_3jetsExactlyPlusHighPtAK8);  mySonic.ScheduleVariablesForRegion("=3jets+highPtAK8","all");
     
     // >= 4 jets
     mySonic.AddRegion(">=4jets",           "",&Selector_4jets);                      mySonic.ScheduleVariablesForRegion(">=4jets","all");
     mySonic.AddRegion(">=4jets+Wtag",      "",&Selector_4jetsPlusWtag);              mySonic.ScheduleVariablesForRegion(">=4jets+Wtag","all");
     mySonic.AddRegion(">=4jets+highPtAK8", "",&Selector_4jetsPlusHighPtAK8);         mySonic.ScheduleVariablesForRegion(">=4jets+highPtAK8","all");

     // >= 4 jets + veto
     mySonic.AddRegion(">=4jets+vetoWtag",  "",&Selector_4jetsPlusVetoWtag);          mySonic.ScheduleVariablesForRegion(">=4jets+vetoWtag","all");
     mySonic.AddRegion(">=4jets+vetoHighPtAK8", "",&Selector_4jetsPlusVetoHighPtAK8); mySonic.ScheduleVariablesForRegion(">=4jets+vetoHighPtAK8","all");
     
     // for info..
     mySonic.AddRegion(">=3jets+highPtGenW","",&Selector_3jetsPlusHighPtGenW);        mySonic.ScheduleVariablesForRegion(">=3jets+highPtGenW","all");
  ...
  // ########################################
	 ...
	 // Create histograms
  	 ...
  	 mySonic.Add2DHisto("MT","recoW_dR");
...
//     mySonic.SchedulePlots("3DProjectedTo2D","varX=mStop,varY=mNeutralino,varZ=genW_Pt,projectionType=fracSupTo200,labelZ=frac(genW_Pt>200)");
  ...
  cout << endl;
      ...
      threeJetsSelections.push_back(">=3jets+highPtAK8");
      ...
      threeJetsExactlySelections.push_back("=3jets+highPtAK8");
      
      vector<string> fourJetsSelections;
      fourJetsSelections.push_back(">=4jets");
      fourJetsSelections.push_back(">=4jets+Wtag");
      fourJetsSelections.push_back(">=4jets+highPtAK8");

      vector<string> fourJetsWithVetoSelections;
      fourJetsWithVetoSelections.push_back(">=4jets");
      fourJetsWithVetoSelections.push_back(">=4jets+vetoWtag");
      fourJetsWithVetoSelections.push_back(">=4jets+vetoHighPtAK8");
      ...
      Table fourJetsWithVeto(fourJetsWithVetoSelections,processClassList);
      ...
      fillMCSignalTable(&mySonic,fourJetsWithVetoSelections,processClassList,&fourJetsWithVeto);

      reference.PrintTable();
      threeJets.PrintTable();
      threeJetsExactly.PrintTable();
      fourJets.PrintTable();
      fourJetsWithVeto.PrintTable();

      TH1F* reference__bkg = new TH1F("reference__bkg","",2,0.5,2.5);
      TH1F* reference__sig = new TH1F("reference__sig","",2,0.5,2.5);
      
      TH1F* reference2__bkg = new TH1F("reference2__bkg","",2,0.5,2.5);
      TH1F* reference2__sig = new TH1F("reference2__sig","",2,0.5,2.5);
      
      TH1F* scenarioA_Wtag__bkg = new TH1F("scenarioA_Wtag__bkg","",2,0.5,2.5);
      TH1F* scenarioA_Wtag__sig = new TH1F("scenarioA_Wtag__sig","",2,0.5,2.5);
      
      TH1F* scenarioA_highPtAK8__bkg = new TH1F("scenarioA_highPtAK8__bkg","",2,0.5,2.5);
      TH1F* scenarioA_highPtAK8__sig = new TH1F("scenarioA_highPtAK8__sig","",2,0.5,2.5);

      TH1F* scenarioB_Wtag__bkg = new TH1F("scenarioB_Wtag__bkg","",2,0.5,2.5);
      TH1F* scenarioB_Wtag__sig = new TH1F("scenarioB_Wtag__sig","",2,0.5,2.5);
      
      TH1F* scenarioB_highPtAK8__bkg = new TH1F("scenarioB_highPtAK8__bkg","",2,0.5,2.5);
      TH1F* scenarioB_highPtAK8__sig = new TH1F("scenarioB_highPtAK8__sig","",2,0.5,2.5);

      TH1F* scenarioC_Wtag__bkg = new TH1F("scenarioC_Wtag__bkg","",2,0.5,2.5);
      TH1F* scenarioC_Wtag__sig = new TH1F("scenarioC_Wtag__sig","",2,0.5,2.5);
      
      TH1F* scenarioC_highPtAK8__bkg = new TH1F("scenarioC_highPtAK8__bkg","",2,0.5,2.5);
      TH1F* scenarioC_highPtAK8__sig = new TH1F("scenarioC_highPtAK8__sig","",2,0.5,2.5);


      // Reference selection    | One box | >= 4 jets, no requirement on W-tag or AK8 
      
      reference__bkg->Fill(1.0,fourJets.Get(">=4jets","total").value());
      reference__sig->Fill(1.0,fourJets.Get(">=4jets","signal").value()/9.0);

      //                                     [1] >= 4 jets
      // Reference2 - WTag     | Two box |  
      //                                     [2] == 3 jets
      
      reference2__bkg->Fill(1.0,fourJets.Get(">=4jets","total").value());
      reference2__sig->Fill(1.0,fourJets.Get(">=4jets","signal").value()/9.0);
      reference2__bkg->Fill(2.0,threeJetsExactly.Get("=3jets","total").value());
      reference2__sig->Fill(2.0,threeJetsExactly.Get("=3jets","signal").value()/9.0);

      //                                     [1] >= 4 jets
      // Scenario A - WTag      | Two box |  
      //                                     [2] == 3 jets + at least one Wtag

      scenarioA_Wtag__bkg->Fill(1.0,fourJets.Get(">=4jets","total").value());
      scenarioA_Wtag__sig->Fill(1.0,fourJets.Get(">=4jets","signal").value()/9.0);
      scenarioA_Wtag__bkg->Fill(2.0,threeJetsExactly.Get("=3jets+Wtag","total").value());
      scenarioA_Wtag__sig->Fill(2.0,threeJetsExactly.Get("=3jets+Wtag","signal").value()/9.0);

      //                                     [1] >= 4 jets
      // Scenario A - highPtAK8 | Two box |  
      //                                     [2] == 3 jets + at least one high Pt (>200) AK8 jet

      scenarioA_highPtAK8__bkg->Fill(1.0,fourJets.Get(">=4jets","total").value());
      scenarioA_highPtAK8__sig->Fill(1.0,fourJets.Get(">=4jets","signal").value()/9.0);
      scenarioA_highPtAK8__bkg->Fill(2.0,threeJetsExactly.Get("=3jets+highPtAK8","total").value());
      scenarioA_highPtAK8__sig->Fill(2.0,threeJetsExactly.Get("=3jets+highPtAK8","signal").value()/9.0);

      //                                     [1] >= 4 jets + veto on Wtag
      // Scenario B - WTag      | Two box |  
      //                                     [2] >= 3 jets + at least one Wtag

      scenarioB_Wtag__bkg->Fill(1.0,fourJetsWithVeto.Get(">=4jets+vetoWtag","total").value());
      scenarioB_Wtag__sig->Fill(1.0,fourJetsWithVeto.Get(">=4jets+vetoWtag","signal").value()/9.0);
      scenarioB_Wtag__bkg->Fill(2.0,threeJets.Get(">=3jets+Wtag","total").value());
      scenarioB_Wtag__sig->Fill(2.0,threeJets.Get(">=3jets+Wtag","signal").value()/9.0);

      //                                     [1] >= 4 jets + veto on high Pt (>200) AK8 jet
      // Scenario B - highPtAK8 | Two box |  
      //                                     [2] >= 3 jets + at least one high Pt (>200) AK8 jet

      scenarioB_highPtAK8__bkg->Fill(1.0,fourJetsWithVeto.Get(">=4jets+vetoHighPtAK8","total").value());
      scenarioB_highPtAK8__sig->Fill(1.0,fourJetsWithVeto.Get(">=4jets+vetoHighPtAK8","signal").value()/9.0);
      scenarioB_highPtAK8__bkg->Fill(2.0,threeJets.Get(">=3jets+highPtAK8","total").value());
      scenarioB_highPtAK8__sig->Fill(2.0,threeJets.Get(">=3jets+highPtAK8","signal").value()/9.0);
     
      //                                     [1] >= 4 jets + veto on Wtag
      // Scenario C - WTag      | Two box |  
      //                                     [2] >= 4 jets + at least one Wtag

      scenarioC_Wtag__bkg->Fill(1.0,fourJetsWithVeto.Get(">=4jets+vetoWtag","total").value());
      scenarioC_Wtag__sig->Fill(1.0,fourJetsWithVeto.Get(">=4jets+vetoWtag","signal").value()/9.0);
      scenarioC_Wtag__bkg->Fill(2.0,fourJets.Get(">=4jets+Wtag","total").value());
      scenarioC_Wtag__sig->Fill(2.0,fourJets.Get(">=4jets+Wtag","signal").value()/9.0);

      //                                     [1] >= 4 jets + veto on high Pt (>200) AK8 jet
      // Scenario C - highPtAK8 | Two box |  
      //                                     [2] >= 4 jets + at least one high Pt (>200) AK8 jet

      scenarioC_highPtAK8__bkg->Fill(1.0,fourJetsWithVeto.Get(">=4jets+vetoHighPtAK8","total").value());
      scenarioC_highPtAK8__sig->Fill(1.0,fourJetsWithVeto.Get(">=4jets+vetoHighPtAK8","signal").value()/9.0);
      scenarioC_highPtAK8__bkg->Fill(2.0,fourJets.Get(">=4jets+highPtAK8","total").value());
      scenarioC_highPtAK8__sig->Fill(2.0,fourJets.Get(">=4jets+highPtAK8","signal").value()/9.0);

      TFile scenariosSave("scenarios.root","RECREATE");
    
      reference__bkg->Write();
      reference__sig->Write();
      
      reference2__bkg->Write();
      reference2__sig->Write();

      scenarioA_Wtag__bkg->Write();
      scenarioA_Wtag__sig->Write();
    
      scenarioA_highPtAK8__bkg->Write();
      scenarioA_highPtAK8__sig->Write();

      scenarioB_Wtag__bkg->Write();
      scenarioB_Wtag__sig->Write();
      
      scenarioB_highPtAK8__bkg->Write();
      scenarioB_highPtAK8__sig->Write();

      scenarioC_Wtag__bkg->Write();
      scenarioC_Wtag__sig->Write();
      
      scenarioC_highPtAK8__bkg->Write();
      scenarioC_highPtAK8__sig->Write();
