...
bool NLeptonSkim(IPHCTree::NTEvent * event, int Nlep, int Nj)
{
    ...
    // -------------------------------------
    {
      ...
      {
         if (event->electrons[i].p4.Pt() > 20) nElectrons++;
      }
    ...
    // -------------------------------------
    {
      ...
      {
         if (event->muons[i].p4.Pt() > 20) nMuons++;
      }
    ...
    cout << "Nlep= " << nElectrons+nMuons << " Njet = " << nJets << endl;
...
//  std::cout << "#########################" << std::endl;
  ...
  if(argc<1) { std::cout << "Syntax is : NTupleSkimmer filename"  << std::endl; return 1;}
  
  // Input file
  TString nTupleName = argv[1];

  //TString file = "/opt/sbg/data/data1/cms/aaubin/"+dataset+"/"+nTupleName;
  TString file = "/opt/sbg/data/data4/cms/aaubin/tauVetoValidation_beta/CMSSW_5_3_2_patch4/src/NTuple/NTupleAnalysis/macros/TTbarMET/"+nTupleName;
 
  stringstream nLepInput_;      nLepInput_ << argv[2];
  int nLepInput;                nLepInput_ >> nLepInput;
  
  stringstream nJetInput_;      nJetInput_ << argv[3];
  int nJetInput;                nJetInput_ >> nJetInput;

  // Output file
  //TString outFileName = "/opt/sbg/data/data1/cms/aaubin/"+dataset+"/skimmed_"+nTupleName;
  TString outFileName = "/opt/sbg/data/data4/cms/aaubin/tauVetoValidation_beta/CMSSW_5_3_2_patch4/src/NTuple/NTupleAnalysis/macros/TTbarMET/skimmed"+nTupleName;
  ...
  //  if (ievt%100000==0) std::cout << "ievt=" << ievt << std::endl;
    ...
    if(!NLeptonSkim(event, nLepInput, nJetInput )) continue; 
    nselected++;
