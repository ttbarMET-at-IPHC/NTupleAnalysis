...
bool WTagged_Randomize(float mass, float pT, float dR_lepton, float dR_gen0, float sigmaUncertaintyTagging, float sigmaUncertantyMistag)
{    
    if (pT < 200) return false;
    else if ((dR_gen < 0.1) && (r0->Rndm() < 0.6 * (1 + sigmaUncertaintyTagging*0.02)))
        return true;
    else if ((dR_gen > 0.1) && (r0->Rndm() < 0.1 * (1 + sigmaUncertaintyMistag*0.3)))
        return true;
    else
        return false;
}
...
bool Selector_AtLeastOneWTag(float sigmaUncertaintyTagging, float sigmaUncertaintyMistag)
{
    for (int i = 0 ; i < 10 ; i++)
    {
/*
...
*/
        float dR_lep = deltaR(myEventPointer->lep_eta,      myEventPointer->lep_phi,
                              myEventPointer->recoW_Eta[i], myEventPointer->recoW_Phi[i])
        float dR_gen = deltaR(myEventPointer->genW_Eta,     myEventPointer->genW_Phi
                              myEventPointer->recoW_Eta[i], myEventPointer->recoW_Phi[i]);

        if (WTagged_Randomize(mass,pT,dR_lep,dR_gen,sigmaUncertaintyTagging,sigmaUncertaintyMistag))
            return true;
    }
...
bool Selector_3jetsPlusWtag()            { if (Selector_AtLeastOneWTag(0))    { return Selector_3jets();        } else return false; }
bool Selector_3jetsExactlyPlusWtag()     { if (Selector_AtLeastOneWTag(0))    { return Selector_3jetsExactly(); } else return false; }
bool Selector_4jetsPlusWtag()            { if (Selector_AtLeastOneWTag(0))    { return Selector_4jets();        } else return false; }
bool Selector_3jetsPlusVetoWtag()        { if (!Selector_AtLeastOneWTag(0))   { return Selector_3jets();        } else return false; }
bool Selector_3jetsExactlyPlusVetoWtag() { if (!Selector_AtLeastOneWTag(0))   { return Selector_3jetsExactly(); } else return false; }
bool Selector_4jetsPlusVetoWtag()        { if (!Selector_AtLeastOneWTag(0))   { return Selector_4jets();        } else return false; }

bool Selector_3jetsPlusWtag_Plus1sigTag()            { if (Selector_AtLeastOneWTag(1,0))    { return Selector_3jets();        } else return false; }
bool Selector_3jetsExactlyPlusWtag_Plus1sigTag()     { if (Selector_AtLeastOneWTag(1,0))    { return Selector_3jetsExactly(); } else return false; }
bool Selector_4jetsPlusWtag_Plus1sigTag()            { if (Selector_AtLeastOneWTag(1,0))    { return Selector_4jets();        } else return false; }
bool Selector_3jetsPlusVetoWtag_Plus1sigTag()        { if (!Selector_AtLeastOneWTag(1,0))   { return Selector_3jets();        } else return false; }
bool Selector_3jetsExactlyPlusVetoWtag_Plus1sigTag() { if (!Selector_AtLeastOneWTag(1,0))   { return Selector_3jetsExactly(); } else return false; }
bool Selector_4jetsPlusVetoWtag_Plus1sigTag()        { if (!Selector_AtLeastOneWTag(1,0))   { return Selector_4jets();        } else return false; }

bool Selector_3jetsPlusWtag_Minus1sigTag()            { if (Selector_AtLeastOneWTag(-1,0))    { return Selector_3jets();        } else return false; }
bool Selector_3jetsExactlyPlusWtag_Minus1sigTag()     { if (Selector_AtLeastOneWTag(-1,0))    { return Selector_3jetsExactly(); } else return false; }
bool Selector_4jetsPlusWtag_Minus1sigTag()            { if (Selector_AtLeastOneWTag(-1,0))    { return Selector_4jets();        } else return false; }
bool Selector_3jetsPlusVetoWtag_Minus1sigTag()        { if (!Selector_AtLeastOneWTag(-1,0))   { return Selector_3jets();        } else return false; }
bool Selector_3jetsExactlyPlusVetoWtag_Minus1sigTag() { if (!Selector_AtLeastOneWTag(-1,0))   { return Selector_3jetsExactly(); } else return false; }
bool Selector_4jetsPlusVetoWtag_Minus1sigTag()        { if (!Selector_AtLeastOneWTag(-1,0))   { return Selector_4jets();        } else return false; }

bool Selector_3jetsPlusWtag_Plus1sigMis()            { if (Selector_AtLeastOneWTag(0,1))    { return Selector_3jets();        } else return false; }
bool Selector_3jetsExactlyPlusWtag_Plus1sigMis()     { if (Selector_AtLeastOneWTag(0,1))    { return Selector_3jetsExactly(); } else return false; }
bool Selector_4jetsPlusWtag_Plus1sigMis()            { if (Selector_AtLeastOneWTag(0,1))    { return Selector_4jets();        } else return false; }
bool Selector_3jetsPlusVetoWtag_Plus1sigMis()        { if (!Selector_AtLeastOneWTag(0,1))   { return Selector_3jets();        } else return false; }
bool Selector_3jetsExactlyPlusVetoWtag_Plus1sigMis() { if (!Selector_AtLeastOneWTag(0,1))   { return Selector_3jetsExactly(); } else return false; }
bool Selector_4jetsPlusVetoWtag_Plus1sigMis()        { if (!Selector_AtLeastOneWTag(0,1))   { return Selector_4jets();        } else return false; }

bool Selector_3jetsPlusWtag_Minus1sigMis()            { if (Selector_AtLeastOneWTag(0,-1))    { return Selector_3jets();        } else return false; }
bool Selector_3jetsExactlyPlusWtag_Minus1sigMis()     { if (Selector_AtLeastOneWTag(0,-1))    { return Selector_3jetsExactly(); } else return false; }
bool Selector_4jetsPlusWtag_Minus1sigMis()            { if (Selector_AtLeastOneWTag(0,-1))    { return Selector_4jets();        } else return false; }
bool Selector_3jetsPlusVetoWtag_Minus1sigMis()        { if (!Selector_AtLeastOneWTag(0,-1))   { return Selector_3jets();        } else return false; }
bool Selector_3jetsExactlyPlusVetoWtag_Minus1sigMis() { if (!Selector_AtLeastOneWTag(0,-1))   { return Selector_3jetsExactly(); } else return false; }
bool Selector_4jetsPlusVetoWtag_Minus1sigMis()        { if (!Selector_AtLeastOneWTag(0,-1))   { return Selector_4jets();        } else return false; }
...
int main (int argc, char *argv[])
{
  ...
  // ##########################
     ...
     // +1sigmaTag
     mySonic.AddRegion(">=3jets+Wtag_Plus1sigTag",          "",             &Selector_3jetsPlusWtag_Plus1sigTag);
     mySonic.AddRegion(">=3jets+vetoWtag_Plus1sigTag",      "",             &Selector_3jetsPlusVetoWtag_Plus1sigTag);
     mySonic.AddRegion("=3jets+Wtag_Plus1sigTag",           "",             &Selector_3jetsExactlyPlusWtag_Plus1sigTag);
     mySonic.AddRegion("=3jets+vetoWtag_Plus1sigTag",       "",             &Selector_3jetsExactlyPlusVetoWtag_Plus1sigTag);
     mySonic.AddRegion(">=4jets+Wtag_Plus1sigTag",          "",             &Selector_4jetsPlusWtag_Plus1sigTag);
     mySonic.AddRegion(">=4jets+vetoWtag_Plus1sigTag",      "",             &Selector_4jetsPlusVetoWtag_Plus1sigTag);
 
     // -1sigmaTag 
     mySonic.AddRegion(">=3jets+Wtag_Minus1sigTag",          "",             &Selector_3jetsPlusWtag_Minus1sigTag);
     mySonic.AddRegion(">=3jets+vetoWtag_Minus1sigTag",      "",             &Selector_3jetsPlusVetoWtag_Minus1sigTag);
     mySonic.AddRegion("=3jets+Wtag_Minus1sigTag",           "",             &Selector_3jetsExactlyPlusWtag_Minus1sigTag);
     mySonic.AddRegion("=3jets+vetoWtag_Minus1sigTag",       "",             &Selector_3jetsExactlyPlusVetoWtag_Minus1sigTag);
     mySonic.AddRegion(">=4jets+Wtag_Minus1sigTag",          "",             &Selector_4jetsPlusWtag_Minus1sigTag);
     mySonic.AddRegion(">=4jets+vetoWtag_Minus1sigTag",      "",             &Selector_4jetsPlusVetoWtag_Minus1sigTag);
 
     // +1sigmaTag
     mySonic.AddRegion(">=3jets+Wtag_Plus1sigMis",          "",             &Selector_3jetsPlusWtag_Plus1sigMis);
     mySonic.AddRegion(">=3jets+vetoWtag_Plus1sigMis",      "",             &Selector_3jetsPlusVetoWtag_Plus1sigMis);
     mySonic.AddRegion("=3jets+Wtag_Plus1sigMis",           "",             &Selector_3jetsExactlyPlusWtag_Plus1sigMis);
     mySonic.AddRegion("=3jets+vetoWtag_Plus1sigMis",       "",             &Selector_3jetsExactlyPlusVetoWtag_Plus1sigMis);
     mySonic.AddRegion(">=4jets+Wtag_Plus1sigMis",          "",             &Selector_4jetsPlusWtag_Plus1sigMis);
     mySonic.AddRegion(">=4jets+vetoWtag_Plus1sigMis",      "",             &Selector_4jetsPlusVetoWtag_Plus1sigMis);
 
     // -1sigmaTag 
     mySonic.AddRegion(">=3jets+Wtag_Minus1sigMis",          "",             &Selector_3jetsPlusWtag_Minus1sigMis);
     mySonic.AddRegion(">=3jets+vetoWtag_Minus1sigMis",      "",             &Selector_3jetsPlusVetoWtag_Minus1sigMis);
     mySonic.AddRegion("=3jets+Wtag_Minus1sigMis",           "",             &Selector_3jetsExactlyPlusWtag_Minus1sigMis);
     mySonic.AddRegion("=3jets+vetoWtag_Minus1sigMis",       "",             &Selector_3jetsExactlyPlusVetoWtag_Minus1sigMis);
     mySonic.AddRegion(">=4jets+Wtag_Minus1sigMis",          "",             &Selector_4jetsPlusWtag_Minus1sigMis);
     mySonic.AddRegion(">=4jets+vetoWtag_Minus1sigMis",      "",             &Selector_4jetsPlusVetoWtag_Minus1sigMis);
  ...
  cout << endl;
    ...
    for (unsigned int i = 0 ; i < vectorOfHisto2D->size() ; i++)
    {
         ...
         && (currentHisto.getVariableYTag() == "mNeutralino"))
        {
                ...
                if (currentHisto.getRegionTag() == ">=3jets+Wtag_Plus1sigTag")
                if (currentHisto.getRegionTag() == ">=3jets+vetoWtag_Plus1sigTag")
                if (currentHisto.getRegionTag() == "=3jets+Wtag_Plus1sigTag")
                if (currentHisto.getRegionTag() == "=3jets+vetoWtag_Plus1sigTag")
                if (currentHisto.getRegionTag() == ">=4jets+Wtag_Plus1sigTag")
                if (currentHisto.getRegionTag() == ">=4jets+vetoWtag_Plus1sigTag")
                if (currentHisto.getRegionTag() == ">=3jets+Wtag_Minus1sigTag")
                if (currentHisto.getRegionTag() == ">=3jets+vetoWtag_Minus1sigTag")
                if (currentHisto.getRegionTag() == "=3jets+Wtag_Minus1sigTag")
                if (currentHisto.getRegionTag() == "=3jets+vetoWtag_Minus1sigTag")
                if (currentHisto.getRegionTag() == ">=4jets+Wtag_Minus1sigTag")
                if (currentHisto.getRegionTag() == ">=4jets+vetoWtag_Minus1sigTag")
                if (currentHisto.getRegionTag() == ">=3jets+Wtag_Plus1sigMis")
                if (currentHisto.getRegionTag() == ">=3jets+vetoWtag_Plus1sigMis")
                if (currentHisto.getRegionTag() == "=3jets+Wtag_Plus1sigMis")
                if (currentHisto.getRegionTag() == "=3jets+vetoWtag_Plus1sigMis")
                if (currentHisto.getRegionTag() == ">=4jets+Wtag_Plus1sigMis")
                if (currentHisto.getRegionTag() == ">=4jets+vetoWtag_Plus1sigMis")
                if (currentHisto.getRegionTag() == ">=3jets+Wtag_Minus1sigMis")
                if (currentHisto.getRegionTag() == ">=3jets+vetoWtag_Minus1sigMis")
                if (currentHisto.getRegionTag() == "=3jets+Wtag_Minus1sigMis")
                if (currentHisto.getRegionTag() == "=3jets+vetoWtag_Minus1sigMis")
                if (currentHisto.getRegionTag() == ">=4jets+Wtag_Minus1sigMis")
                if (currentHisto.getRegionTag() == ">=4jets+vetoWtag_Minus1sigMis")
