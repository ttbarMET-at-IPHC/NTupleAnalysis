...
int main()
{
  ...
  //mySonic.AddVariable("MET","MET","GeV",30,50,500,&(event.MET));
  //mySonic.AddVariable("MT","MT","GeV",25,0,500,&(event.MT));
  ...
  mySonic.AddVariable("MET_preTauVeto",         "MET",                          "GeV",30,50,500,    0,  "logY=true");
  mySonic.AddVariable("MET_postTauVeto",        "MET",                          "GeV",30,50,500,    0,  "logY=true");
  mySonic.AddVariable("MET_CR5tau",             "MET",                          "GeV",30,50,500,    0,  "logY=true");
  
  mySonic.AddVariable("MT_CR5track",            "MT",                           "GeV",17,-10,500,     0,  "logY=true");
  mySonic.AddVariable("MT_preTauVeto",          "MT",                           "GeV",17,-10,500,     0,  "logY=true");
  mySonic.AddVariable("MT_postTauVeto",         "MT",                           "GeV",17,-10,500,     0,  "logY=true");
  mySonic.AddVariable("MT_CR5tau",              "MT",                           "GeV",17,-10,500,     0,  "logY=true");
  
  
  mySonic.AddVariable("MTpeak-preTrackVeto",    "MT_{peak} (before track veto)","GeV",10,10,110,0);
  mySonic.AddVariable("MTpeak-CR5track",          "MT_{peak} (after all vetos)",  "GeV",10,10,110,0);
  mySonic.AddVariable("MTpeak-CR5track_2",        "MT_{peak} (after all vetos)",  "GeV",1,50,80,0);
  
  mySonic.AddVariable("MTpeak-preTauVeto",      "MT_{peak} (before tau veto)",  "GeV",10,10,110,0);
  mySonic.AddVariable("MTpeak-postVeto",        "MT_{peak} (after all vetos)",  "GeV",10,10,110,0);
  mySonic.AddVariable("MTpeak-CR5tau",          "MT_{peak} (after all vetos)",  "GeV",10,10,110,0);
  mySonic.AddVariable("MTpeak-CR5tau_2",        "MT_{peak} (after all vetos)",  "GeV",1,50,80,0);
 

  mySonic.AddVariable("pT_goodTau",             "p_{T}(#tau-vetoed)",           "GeV",12,0,240,0,       "logY=true");
  
  mySonic.AddVariable("MT-preTauVeto-region1",      "MT",                       "GeV",20,0,500,0);
  mySonic.AddVariable("MT-postTauVeto-region1",     "MT",                       "GeV",20,0,500,0);
  
  mySonic.AddVariable("MT-preTauVeto-region2",      "MT",                       "GeV",20,0,500,0);
  mySonic.AddVariable("MT-postTauVeto-region2",     "MT",                       "GeV",20,0,500,0);
  
  mySonic.AddVariable("DPhi-preTauVeto",       "#Delta#phi(l,MET) (before tau veto)",  "",     10,-3.14,3.14);
  mySonic.AddVariable("HT",                    "HT (before tau veto)",                 "GeV",  20,0,500);
  mySonic.AddVariable("HT_ratio",              "HT_{ratio} (before tau veto)",         "",     10,0,1);
  mySonic.AddVariable("M3",                    "M_{3} (before tau veto)",              "GeV",  20,0,800);
  //mySonic.AddVariable("hadrChi2","hadr.#Chi^{2} (before tau veto)","",10,-3.14,3.14,0);
  
  mySonic.AddVariable("DPhi-preTauVeto-MTpeak","#Delta#phi(l,MET) (before tau veto, MTpeak)",  "",     10,-3.14,3.14);
  mySonic.AddVariable("HT-MTpeak",             "HT (before tau veto, MTpeak)",                 "GeV",  20,0,500);
  mySonic.AddVariable("HT_ratio-MTpeak",       "HT_{ratio} (before tau veto, MTpeak)",         "",     10,0,1);
  mySonic.AddVariable("M3-MTpeak",             "M_{3} (before tau veto, MTpeak)",              "GeV",  20,0,800);
    ...
    for (unsigned int i = 0 ; i < datasetsList.size() ; i++)
  {
      ...
      for (int j = 0 ; j < theTree->GetEntries() ; j++)
      {
        ...
        if (inMTpeak) mySonic.Fill("MTpeak-preTrackVeto",currentProcessClass,event.MT,weight);
          
            // CR5_track
            if (event.isolatedTrackVeto == 1.0)
            {
                mySonic.Fill("MT_CR5track" ,currentProcessClass,event.MT,weight);
                if (inMTpeak) mySonic.Fill("MTpeak-CR5track" ,currentProcessClass,event.MT,weight);
                if (inMTpeak) mySonic.Fill("MTpeak-CR5track_2" ,currentProcessClass,event.MT,weight);
            }

        // Now, require to have no isolated track
        
        if (event.isolatedTrackVeto != 1.0) continue;

        mySonic.AutoFillProcessClass(currentProcessClass,weight);
        
        mySonic.Fill("MET_preTauVeto",currentProcessClass,event.MET,weight);
        mySonic.Fill("MT_preTauVeto" ,currentProcessClass,event.MT,weight);
       
        mySonic.Fill("DPhi-preTauVeto",currentProcessClass,event.MET_lep_dphi,weight);
        mySonic.Fill("HT",currentProcessClass,event.HT,weight);
        mySonic.Fill("HT_ratio",currentProcessClass,event.HT_ratio,weight);
        mySonic.Fill("M3",currentProcessClass,event.M3,weight);

        if (inMTpeak)
        {
            mySonic.Fill("MTpeak-preTauVeto",currentProcessClass,event.MT,weight);
            
            mySonic.Fill("DPhi-preTauVeto-MTpeak",currentProcessClass,event.MET_lep_dphi,weight);
            mySonic.Fill("HT-MTpeak",currentProcessClass,event.HT,weight);
            mySonic.Fill("HT_ratio-MTpeak",currentProcessClass,event.HT_ratio,weight);
            mySonic.Fill("M3-MTpeak",currentProcessClass,event.M3,weight);
        }
        
        if (inRegion1) mySonic.Fill("MT-preTauVeto-region1",currentProcessClass,event.MT,weight);
        if (inRegion2) mySonic.Fill("MT-preTauVeto-region2",currentProcessClass,event.MT,weight);
        ...
        if (event.tauVeto == 1.0)
        {
            ...
            if (inMTpeak) mySonic.Fill("MTpeak-CR5tau" ,currentProcessClass,event.MT,weight);
            if (inMTpeak) mySonic.Fill("MTpeak-CR5tau_2" ,currentProcessClass,event.MT,weight);
...
*/
            ...
            mySonic.Fill("MET_postTauVeto",currentProcessClass,event.MET,weight);
            mySonic.Fill("MT_postTauVeto" ,currentProcessClass,event.MT,weight);

            if (inMTpeak)  mySonic.Fill("MTpeak-postVeto",currentProcessClass,event.MT,weight);
            if (inRegion1) mySonic.Fill("MT-postTauVeto-region1",currentProcessClass,event.MT,weight);
            if (inRegion2) mySonic.Fill("MT-postTauVeto-region2",currentProcessClass,event.MT,weight);
         }
      }
  ...
  // #######################
      ...
      // Apply tauId-scaleFactor on dilepton
      FigureScrew tauID_scaleFactor(1.0,0.07);
      
 //     FigureScrew efficaciteVeto(0.2337,0.0084);

//      FigureScrew tauVeto_scaleFactor = (FigureScrew(1.0,0.0) - efficaciteVeto * tauID_scaleFactor) / (FigureScrew(1.0,0.0) - tauID_scaleFactor);
      FigureScrew tauVeto_scaleFactor(1.0,0.03);
  ...
  FigureScrew yieldTotalCR5track = 0.0;
  ...
  for (int i = 0 ; i < processClassesList.size() ; i++)
  {
     ...
     tmpYield = mySonic.GetYieldAndError("MTpeak-preTrackVeto",processClassesList[i]);
     yieldsMTPeak.Set("pre-track-veto",processClassesList[i],tmpYield);
     if (processClassesList[i] != "data") yieldTotalPreTrack += tmpYield;

     tmpYield = mySonic.GetYieldAndError("MTpeak-CR5track",processClassesList[i]);
     yieldsMTPeak.Set("CR5track",processClassesList[i],tmpYield);
     if (processClassesList[i] != "data") yieldTotalCR5track += tmpYield;

     tmpYield = mySonic.GetYieldAndError("MTpeak-preTauVeto",processClassesList[i]);
     yieldsMTPeak.Set("pre-tau-veto",processClassesList[i],tmpYield);
     if (processClassesList[i] != "data") yieldTotalPreTau += tmpYield;

     tmpYield = mySonic.GetYieldAndError("MTpeak-postVeto",processClassesList[i]);
     yieldsMTPeak.Set("post-veto",processClassesList[i],tmpYield);
     if (processClassesList[i] != "data") yieldTotalPost += tmpYield;
     
     tmpYield = mySonic.GetYieldAndError("MTpeak-CR5tau",processClassesList[i]);
  ...
  yieldsMTPeak.Set("CR5track",      "total MC", yieldTotalCR5track);
  ...
  FigureScrew preTrack_data  = yieldsMTPeak.Get("pre-track-veto","data");
  FigureScrew preTrack_rare  = yieldsMTPeak.Get("pre-track-veto","rare");
  FigureScrew preTrack_totMC = yieldsMTPeak.Get("pre-track-veto","total MC");
  
  FigureScrew SFpre_track = (preTrack_data  - preTrack_rare) 
                          / (preTrack_totMC - preTrack_rare);

  // SFCR5_track
   
  FigureScrew CR5track_data  = yieldsMTPeak.Get("CR5track","data");
  FigureScrew CR5track_rare  = yieldsMTPeak.Get("CR5track","rare");
  FigureScrew CR5track_totMC = yieldsMTPeak.Get("CR5track","total MC");
  FigureScrew CR5track_ll    = yieldsMTPeak.Get("CR5track","ttbarFullLept");
 
  FigureScrew SFCR5_track = (CR5track_data  - CR5track_rare - SFpre_track*CR5track_ll)
                          / (CR5track_totMC - CR5track_rare - CR5track_ll);

  // SFpost_track
  
  FigureScrew preTau_data  = yieldsMTPeak.Get("pre-tau-veto","data");
  FigureScrew preTau_rare  = yieldsMTPeak.Get("pre-tau-veto","rare");
  FigureScrew preTau_totMC = yieldsMTPeak.Get("pre-tau-veto","total MC");
  FigureScrew preTau_ll    = yieldsMTPeak.Get("pre-tau-veto","ttbarFullLept");
 
  FigureScrew SFpost_track = (preTau_data  - preTau_rare - SFpre_track*preTau_ll)
                           / (preTau_totMC - preTau_rare - preTau_ll);


  // SFpre_tau
  
  FigureScrew SFpre_tau =                      (preTau_data  - preTau_rare)
                           / (   ((preTau_totMC - preTau_rare - preTau_ll) * SFpost_track) 
                               + (preTau_ll * SFpre_track)                               );

  // SFCR5_tau
   
  FigureScrew CR5tau_data  = yieldsMTPeak.Get("CR5tau","data");
  FigureScrew CR5tau_rare  = yieldsMTPeak.Get("CR5tau","rare");
  FigureScrew CR5tau_totMC = yieldsMTPeak.Get("CR5tau","total MC");
  FigureScrew CR5tau_ll    = yieldsMTPeak.Get("CR5tau","ttbarFullLept");
 
  FigureScrew SFCR5_tau =  (CR5tau_data  - CR5tau_rare - SFpre_tau * SFpre_track * tauID_scaleFactor * CR5tau_ll)
                        / ((CR5tau_totMC - CR5tau_rare - CR5tau_ll) * SFpost_track) ;


  // SFpost_tau
  
  FigureScrew postTau_data  = yieldsMTPeak.Get("post-veto","data");
  FigureScrew postTau_rare  = yieldsMTPeak.Get("post-veto","rare");
  FigureScrew postTau_totMC = yieldsMTPeak.Get("post-veto","total MC");
  FigureScrew postTau_ll    = yieldsMTPeak.Get("post-veto","ttbarFullLept");


  FigureScrew SFpost_tau =    (postTau_data   - postTau_rare - postTau_ll * SFpre_track * SFpre_tau * tauVeto_scaleFactor)   
                            / ((postTau_totMC - postTau_rare - postTau_ll) * SFpost_track  );

   cout << "SFpre_track  = " << SFpre_track.Print()  << endl;
   cout << "SFpost_track = " << SFpost_track.Print() << endl;
   cout << "SFCR5_track = "  << SFCR5_track.Print()  << endl;
   cout << endl;
   cout << "SFpre_tau  = " << SFpre_tau.Print()   << endl;
   cout << "SFpost_tau = " << SFpost_tau.Print()  << endl;
   cout << "SFCR5_tau = "  << SFCR5_tau.Print()   << endl;

  FigureScrew remaining =  ( postTau_data  - postTau_rare ) 
                        /  (   (postTau_totMC - postTau_rare - postTau_ll) * SFpost_track * SFpost_tau 
                             + (postTau_ll * SFpre_track * SFpre_tau * tauVeto_scaleFactor)
                           );
   ...
   // Apply scale factors
      ...
      cout << "yield dilepton before scale factors (dans CR5tau) = " << mySonic.GetYieldAndError("MT_CR5tau","ttbarFullLept").Print() << endl;
      cout << "tauVeto scale factor = " << tauVeto_scaleFactor.Print() << endl;

      mySonic.ApplyScaleFactor("MT_postTauVeto","ttbarFullLept",tauVeto_scaleFactor);
      mySonic.ApplyScaleFactor("MT_CR5tau","ttbarFullLept",     tauID_scaleFactor);

      // Apply pre-track SF on dilepton
      mySonic.ApplyScaleFactor("MT_CR5track","ttbarFullLept",   SFpre_track);
      mySonic.ApplyScaleFactor("MT_postTauVeto","ttbarFullLept",SFpre_track * SFpre_tau);
      mySonic.ApplyScaleFactor("MT_CR5tau","ttbarFullLept",     SFpre_track * SFpre_tau);
  
      // Apply post-track SF on 1-lep backgrounds
      mySonic.ApplyScaleFactor("MT_CR5track",  "ttbarSemiLept", SFCR5_track);
      mySonic.ApplyScaleFactor("MT_CR5track",  "Singletop",     SFCR5_track);
      mySonic.ApplyScaleFactor("MT_CR5track",  "W+jets",        SFCR5_track);
      mySonic.ApplyScaleFactor("MT_postTauVeto","ttbarSemiLept",SFpost_track * SFpost_tau);
      mySonic.ApplyScaleFactor("MT_postTauVeto","Singletop",    SFpost_track * SFpost_tau);
      mySonic.ApplyScaleFactor("MT_postTauVeto","W+jets",       SFpost_track * SFpost_tau);
      mySonic.ApplyScaleFactor("MT_CR5tau",  "ttbarSemiLept",   SFCR5_tau);
      mySonic.ApplyScaleFactor("MT_CR5tau",  "Singletop",       SFCR5_tau);
      mySonic.ApplyScaleFactor("MT_CR5tau",  "W+jets",          SFCR5_tau);
 
      cout << "yield dilepton after scale factors (dans CR5tau) = " << mySonic.GetYieldAndError("MT_CR5tau","ttbarFullLept").Print() << endl;
  ...
  string region1pre("MT-preTauVeto-region1");
  string region1post("MT-postTauVeto-region1");
  
  string region2pre("MT-preTauVeto-region2");
  string region2post("MT-postTauVeto-region2");
